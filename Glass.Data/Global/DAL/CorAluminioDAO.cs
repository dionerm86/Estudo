...
namespace Glass.Data.DAL
{
    public sealed class CorAluminioDAO : BaseDAO<CorAluminio, CorAluminioDAO>
	{
        ...
        public override int Delete(CorAluminio objDelete)
        {
            if (objPersistence.ExecuteSqlQueryCount("Select * From produto where idCorAluminio=" + objDelete.IdCorAluminio) > 0)
                throw new Exception("Existem produtos associados à esta cor de alumínio, exclua os mesmos antes de excluir esta cor.");

            if (objPersistence.ExecuteSqlQueryCount("Select * From item_projeto where idCorAluminio=" + objDelete.IdCorAluminio) > 0)
