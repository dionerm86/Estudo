...
namespace Glass.Data.DAL
{
    public sealed class ProdutoDAO : BaseCadastroDAO<Produto, ProdutoDAO>
    {
            ...
            out string filtroAdicional)
        {
            ...
            string campos = selecionar ? "p.*, Concat(g.Descricao, ' ', coalesce(sg.descricao, '')) as DescrTipoProduto, " +
                qtdEstoque + " as QtdeEstoque, " + qtdEstoque + " as M2Estoque, " + qtdEntrega + " as QtdeEntrega, " +
                qtdEntrega + " as M2Entrega, '$$$' as criterio" : "Count(*)";

            string sql = "Select " + campos + @" From produto p 
                Inner Join grupo_prod g On (p.idGrupoProd=g.idGrupoProd)
                Left Join subgrupo_prod sg On (p.idSubgrupoProd=sg.idSubgrupoProd)
                Inner Join fornecedor f On (p.idfornec=f.idfornec) Where 1 ?filtroAdicional?";
        ...
        public IList<Produto> GetListDeficit(string codInterno, string descricao, uint idGrupo, uint idSubgrupo, string sortExpression, int startRow, int pageSize)
        {
            string filtroAdicional;
            string sql = SqlDeficit(codInterno, descricao, idGrupo, idSubgrupo, true, out filtroAdicional).
                Replace("?filtroAdicional?", "");

            return LoadDataWithSortExpression(sql, sortExpression, startRow, pageSize, false, filtroAdicional, GetParamDeficit(codInterno, descricao));
        }

        public int GetCountDeficit(string codInterno, string descricao, uint idGrupo, uint idSubgrupo)
        {
            string filtroAdicional;
            string sql = SqlDeficit(codInterno, descricao, idGrupo, idSubgrupo, true, out filtroAdicional).
                Replace("?filtroAdicional?", "");

            return GetCountWithInfoPaging(sql, false, filtroAdicional, GetParamDeficit(codInterno, descricao));
        }

        public IList<Produto> GetRptDeficit(string codInterno, string descricao, uint idGrupo, uint idSubgrupo)
        {
            string filtroAdicional;
            string sql = SqlDeficit(codInterno, descricao, idGrupo, idSubgrupo, true, out filtroAdicional).
                Replace("?filtroAdicional?", filtroAdicional);

            return objPersistence.LoadData(sql, GetParamDeficit(codInterno, descricao)).ToList();
        }

        private GDAParameter[] GetParamDeficit(string codInterno, string descricao)
        {
            List<GDAParameter> lstParam = new List<GDAParameter>();
    ...
    public int ObtemIdGrupoProd(int idProd)
            ...
            IContainerDescontoAcrescimo container, bool revenda)
        {
            ...
            var clienteRevenda = container.IdCliente > 0
                ? ClienteDAO.Instance.IsRevenda(null, container.IdCliente)
                : false;
