...
namespace Glass.Data.DAL
{
    public sealed class InstalacaoDAO : BaseCadastroDAO<Instalacao, InstalacaoDAO>
    {
        ...
        #region Busca instalações para exibir na rota

        /// <summary>
        /// Busca todos as instalações confirmadas da equipe passada entre o período informado
        /// </summary>
        /// <param name="idEquipe"></param>
        /// <param name="dataIni"></param>
        /// <param name="dataFim"></param>
        /// <param name="pendentes">True se for para buscar instalações pendentes de hoje também</param>
        public Instalacao[] GetAllForRota(uint idEquipe, string dataIni, string dataFim, bool pendentes)
        {
            string sql = "Select i.*, group_concat(e.Nome separator '" + SEPARADOR_EQUIPES + "') as NomesEquipes, c.Nome as NomeCliente, l.NomeFantasia as NomeLoja, " +
                "p.DataConf as DataConfPedido, Concat(p.EnderecoObra, ' - ', p.BairroObra, ' - ', p.CidadeObra) as LocalObra " +
                "From instalacao i " +
                ...
                "Where ((i.Confirmada=1 And i.DataConfirmada>=?dataIni And i.DataConfirmada<=?dataFim)";

            sql += pendentes ? " Or (i.DataInstalacao>=?dataIni And i.DataInstalacao<=?dataFim And i.Situacao=" + (int)Instalacao.SituacaoInst.EmAndamento + "))" : ")";

            if (idEquipe > 0)
                sql += " And e.idEquipe=" + idEquipe;

            sql += " group by i.idInstalacao Order By i.Confirmada desc, i.dataConfirmada";

            List<GDAParameter> lstParam = new List<GDAParameter>();
            lstParam.Add(new GDAParameter("?dataIni", DateTime.Parse(dataIni)));
            lstParam.Add(new GDAParameter("?dataFim", DateTime.Parse(dataFim)));

            var lstInst = objPersistence.LoadData(sql, lstParam.ToArray()).ToList().ToArray();

            // Calcula o tempo gasto em cada instalação
            foreach(Instalacao i in lstInst)
            {
                if (i.DataConfirmada == null)
                    continue;

                // Armazena a menor e a maior data dos pontos próximos à instalação
                var menorData = DateTime.Now.AddYears(100);
                var maiorData = DateTime.Now.AddYears(-100);

                foreach (EquipeInstalacao ei in EquipeInstalacaoDAO.Instance.GetByInstalacaoEquipe(i.IdInstalacao, idEquipe))
                {
                    // Busca todos os pontos da equipe da instalacao passada coletados no dia que a mesma foi confirmada
                    var lstPontos = PontosRotaDAO.Instance.GetByEquipe(ei.IdEquipe, i.DataConfirmada.Value.ToString("dd/MM/yyyy 00:00"), i.DataConfirmada.Value.ToString("dd/MM/yyyy 23:59"));

                    // Obtém os pontos próximos ao ponto que foi confirmada a instalação
                    foreach (PontosRota p in lstPontos)
                    {
                        // Se o ponto for próximo do ponto da instalação
                        if (PontoProximo(i.Latitude.Value, i.Longitude.Value, p.Lat, p.Long))
                        {
                            // Se este ponto próximo for menor que o último ponto próximo capturado, salva o mesmo
                            // na variável menorData
                            if (p.DataPonto < menorData)
                                menorData = p.DataPonto;

                            // Se este ponto próximo for maior que o último ponto próximo capturado, salva o mesmo
                            // na variável maiorData
                            if (p.DataPonto > maiorData)
                                maiorData = p.DataPonto;
                        }
                    }
                }

                // Salva o tempo de instalação
                TimeSpan tempoInst = maiorData.Subtract(menorData);
                i.TempoInst = tempoInst.Hours.ToString().PadLeft(2, '0') + ":" + tempoInst.Minutes.ToString().PadLeft(2, '0') + "h";
            }

            return lstInst;
        }

        private bool PontoProximo(decimal lat1, decimal lng1, decimal lat2, decimal lng2)
        {
            // Diferença da longitude do ponto atual para o ponto anterior em módulo
            double difLng = Math.Abs((double)lng1 - (double)lng2);

            // Obtém a distância angular entre a última posição e esta que acaba de ser capturada convertendo a posição
            // para radianos para depois obter o seno e cosseno da mesma
            double curva = Math.Sin((90 - (double)lat2) * 0.0174532925) * Math.Sin((90 - (double)lat1) * 0.0174532925) * Math.Cos(difLng * 0.0174532925) + Math.Cos((90 - (double)lat2) * 0.0174532925) * Math.Cos((90 - (double)lat1) * 0.0174532925);
            double result = Math.Acos(curva);

            // Converte a distância angular para Km, multiplicando a mesma pelo raio da Terra
            result = result * 6371;

            // Se a distância for menor que 70 metros e o resultado for um número válido, retorna verdadeiro
            return result <= 0.08 && result.ToString() != "NaN (Não é um número)" && curva != 1.0000000000000002;
        }
