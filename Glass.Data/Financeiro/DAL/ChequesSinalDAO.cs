...
{
    public sealed class ChequesSinalDAO : BaseCadastroDAO<ChequesSinal, ChequesSinalDAO>
    {
        /// <summary>
        /// Cria um cheque, na tabela cheques_sinal, a partir de uma string.
        /// </summary>
        public void InserirPelaString(GDASession sessao, Sinal sinal, IEnumerable<string> dadosChequesRecebimento)
        {
            ...
            foreach (var dadosChequeRecebimento in dadosChequesRecebimento.Where(f => !string.IsNullOrWhiteSpace(f)))
            {
                ...
                // Insere cheque no BD.
                Insert(sessao, new ChequesSinal
                {
                    IdSinal = (int)sinal.IdSinal,
        ...
        /// <summary>
        /// Recupera a string dos cheques, com base no sinal e com base nos dados inseridos na tabela cheques_sinal.
        /// </summary>
        public string ObterStringChequesPeloSinal(GDASession sessao, int idSinal)
        {
            var chequesSinal = objPersistence.LoadData(sessao, string.Format("SELECT * FROM cheques_sinal WHERE IdSinal={0}", idSinal)).ToList();

            if (chequesSinal == null || chequesSinal.Count == 0)
            {
                return string.Empty;
            }

            return string.Join("|", chequesSinal.Select(f =>
