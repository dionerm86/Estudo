...
{
    public sealed class ChequesAcertoChequeDAO : BaseCadastroDAO<ChequesAcertoCheque, ChequesAcertoChequeDAO>
    {
        /// <summary>
        ...
        /// <summary>
        /// Recupera a string dos cheques, com base no acerto de cheque e com base nos dados inseridos na tabela cheques_acerto_cheque.
        /// </summary>
        public string ObterStringChequesPeloAcertoCheque(GDASession sessao, int idAcertoCheque)
        {
            var chequesAcertoCheque = objPersistence.LoadData(sessao, string.Format("SELECT * FROM cheques_acerto_cheque WHERE IdAcertoCheque={0}", idAcertoCheque)).ToList();

            if (chequesAcertoCheque == null || chequesAcertoCheque.Count == 0)
            {
                return string.Empty;
            }

            return string.Join("|", chequesAcertoCheque.Select(f =>
