...
namespace Glass.Data.DAL
{
    ...
    public sealed partial class PedidoDAO : BaseCadastroDAO<Pedido, PedidoDAO>
    {
        ...
        public int ObtemQuantidadePecasPai(GDASession session, uint idPedido)
        {
            ...
            var sql = $@"SELECT CAST(SUM(COALESCE(Qtde, 0)) AS SIGNED INTEGER) FROM produtos_pedido pp 
                    LEFT JOIN produto p ON (pp.IdProd=p.IdProd)
                WHERE IdPedido=?id AND (Invisivel{invisivel} IS NULL OR Invisivel{invisivel}=0) AND p.IdGrupoProd={(int)NomeGrupoProd.Vidro} AND COALESCE(pp.IdProdPedParent, 0) = 0";
