...
namespace Glass.Data.DAL
{
    public sealed class ProdutosOrcamentoDAO : BaseDAO<ProdutosOrcamento, ProdutosOrcamentoDAO>
    {
            ...
            if (addMateriaisProjeto && showChildren)
            {
                for (int i = lstProd.Count - 1; i >= 0; i--)
                    if (lstProd[i].IdItemProjeto > 0)
                    {
                        ...
                        DescontoAcrescimo.Instance.AplicaAcrescimo(null, 2, lstProd[i].ValorAcrescimo, itens, orcamento);
                        DescontoAcrescimo.Instance.AplicaAcrescimoAmbiente(null, lstProd[i].TipoAcrescimo, lstProd[i].Acrescimo, itens, orcamento);

                        if (PedidoConfig.Comissao.ComissaoAlteraValor)
                            DescontoAcrescimo.Instance.AplicaComissao(null, orcamento.PercComissao, itens, orcamento);

                        if (PedidoConfig.RatearDescontoProdutos)
                        {
                            DescontoAcrescimo.Instance.AplicaDesconto(null, 2, lstProd[i].ValorDesconto, itens, orcamento);
                            DescontoAcrescimo.Instance.AplicaDescontoAmbiente(null, lstProd[i].TipoDesconto, lstProd[i].Desconto, itens, orcamento);
                        }
        ...
        public uint InsereAtualizaProdProj(GDASession sessao, uint idOrcamento, uint? idAmbienteOrca, ItemProjeto itemProj)
        {
            try
            {
                ...
                if (PedidoConfig.Comissao.ComissaoAlteraValor)
                {
                    ...
                    DescontoAcrescimo.Instance.AplicaComissao(sessao, percComissao, prod, orcamento);
                }
        ...
        /// </summary>
        public void AplicaAcrescimo(GDASession session, ProdutosOrcamento produto, Orcamento orcamento)
        {
            ...
            try
            {
                ...
                if (produto.IdItemProjeto == null)
                    produtos.AddRange(GetByProdutoOrcamento(session, produto.IdProd));
                else
                ...
                acrescimoAplicado = DescontoAcrescimo.Instance.AplicaAcrescimoAmbiente(session, produto.TipoAcrescimo, produto.Acrescimo,
                    produtos, orcamento);
                ...
                if (acrescimoAplicado)
                {
                    ...
                    produto.ValorAcrescimoProd = produtos.Sum(p => p.ValorAcrescimoProd);
            ...
            finally
            {
                if (acrescimoAplicado)
                {
                    OrcamentoDAO.Instance.UpdateTotaisOrcamento(session, orcamento);
                }
            }
        }
        ...
        /// </summary>
        public void RemoveAcrescimo(GDASession session, ProdutosOrcamento produto, Orcamento orcamento)
        {
            ...
            try
            {
                ...
                acrescimoRemovido = DescontoAcrescimo.Instance.RemoveAcrescimoAmbiente(session, produtos, orcamento);
        ...
        /// </summary>
        public void AplicaDesconto(GDASession session, ProdutosOrcamento produto, Orcamento orcamento)
        {
            ...
            try
            {
                ...
                descontoAplicado = DescontoAcrescimo.Instance.AplicaDescontoAmbiente(session, produto.TipoDesconto, produto.Desconto, produtos, orcamento);
                ...
                if (descontoAplicado)
                {
                    ...
                    produto.ValorDescontoProd = produtos.Sum(p => p.ValorDescontoProd);
        ...
        /// </summary>
        public void RemoveDesconto(GDASession sessao, ProdutosOrcamento produto, Orcamento orcamento)
        {
            ...
            try
            {
                ...
                if (produto.IdItemProjeto == null)
                    produtos.AddRange(GetByProdutoOrcamento(sessao, produto.IdProd));
                else
                    produtos.Add(produto);

                descontoRemovido = DescontoAcrescimo.Instance.RemoveDescontoAmbiente(sessao, produtos, orcamento);
                ...
                UpdateTotaisProdutoOrcamento(sessao, produto);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public void RecalcularValores(GDASession session, ProdutosOrcamento prod, uint? idCliente, int? tipoEntrega, bool somarAcrescimoDesconto)
        {
            ...
            try
            {
                ...
                ValorBruto.Instance.Calcular(session, prod, orcamento);
                ...
                if (prod.IdProduto > 0)
                {
                    ...
                    var valorUnitario = ValorUnitario.Instance.RecalcularValor(session, prod, orcamento, benef.CountAreaMinimaSession(session) > 0, !somarAcrescimoDesconto);
        ...
        public override uint Insert(GDASession sessao, ProdutosOrcamento objInsert)
        {
            ...
            try
            {
                ...
                if (objInsert.ValorProd > 0 && objInsert.Qtde > 0 && objInsert.Total == null || objInsert.Total == 0)
                {
                    ...
                    DescontoAcrescimo.Instance.AplicaComissao(sessao, orcamento.PercComissao, prod, orcamento);
        ...
        private void CalculaDescontoEValorBrutoProduto(GDASession session, ProdutosOrcamento produto, Orcamento orcamento = null)
        {
            ...
            DescontoAcrescimo.Instance.RemoveDescontoQtde(session, produto, orcamento);
            DescontoAcrescimo.Instance.AplicaDescontoQtde(session, produto, orcamento);
            DiferencaCliente.Instance.Calcular(session, produto, orcamento);
            ValorBruto.Instance.Calcular(session, produto, orcamento);
        }
        ...
        public override int Update(GDASession session, ProdutosOrcamento objUpdate)
        {
            ...
            try
            {
                ...
                {
                    RemoveDesconto(session, prodOrca, orca);
                    AplicaDesconto(session, prodOrca, orca);
                }

                if (atualizarAcrescimo)
                {
                    RemoveAcrescimo(session, prodOrca, orca);
                    AplicaAcrescimo(session, prodOrca, orca);
                }
            ...
            using (var transaction = new GDA.GDATransaction())
            {
                ...
                try
                {
                    ...
                    if (orcamento.Acrescimo > 0)
                        OrcamentoDAO.Instance.RemoveAcrescimo(transaction, orcamento);

                    if (orcamento.Desconto > 0)
                        OrcamentoDAO.Instance.RemoveDesconto(transaction, orcamento);
                    ...
                    if (orcamento.Acrescimo > 0)
                        OrcamentoDAO.Instance.AplicaAcrescimo(transaction, orcamento);

                    if (orcamento.Desconto > 0)
                        OrcamentoDAO.Instance.AplicaDesconto(transaction, orcamento);
        ...
        public int DeleteByPrimaryKeyExcluirProjeto(GDASession session, uint Key)
        {
            ...
            if (orcamento.Acrescimo > 0)
                OrcamentoDAO.Instance.RemoveAcrescimo(session, orcamento);
            
            if (orcamento.Desconto > 0)
                OrcamentoDAO.Instance.RemoveDesconto(session, orcamento);
            ...
            if (orcamento.Acrescimo > 0)
                OrcamentoDAO.Instance.AplicaAcrescimo(session, orcamento);

            if (orcamento.Desconto > 0)
                OrcamentoDAO.Instance.AplicaDesconto(session, orcamento);
