    ...
    public sealed class ProdutosOrcamentoDAO : BaseDAO<ProdutosOrcamento, ProdutosOrcamentoDAO>
    {
            ...
            if (addMateriaisProjeto && showChildren)
            {
                for (int i = lstProd.Count - 1; i >= 0; i--)
                    if (lstProd[i].IdItemProjeto > 0)
                    {
                        ...
                        DescontoAcrescimo.Instance.AplicaAcrescimo(2, lstProd[i].ValorAcrescimo, itens, orcamento);
                        DescontoAcrescimo.Instance.AplicaAcrescimoAmbiente(lstProd[i].TipoAcrescimo, lstProd[i].Acrescimo, itens, orcamento);

                        if (PedidoConfig.Comissao.ComissaoAlteraValor)
                            DescontoAcrescimo.Instance.AplicaComissao(orcamento.PercComissao, itens, orcamento);

                        if (PedidoConfig.RatearDescontoProdutos)
                        {
                            DescontoAcrescimo.Instance.AplicaDesconto(2, lstProd[i].ValorDesconto, itens, orcamento);
                            DescontoAcrescimo.Instance.AplicaDescontoAmbiente(lstProd[i].TipoDesconto, lstProd[i].Desconto, itens, orcamento);
                        }
        ...
        public uint InsereAtualizaProdProj(GDASession sessao, uint idOrcamento, uint? idAmbienteOrca, ItemProjeto itemProj)
        {
            try
            {
                ...
                if (PedidoConfig.Comissao.ComissaoAlteraValor)
                {
                    ...
                    DescontoAcrescimo.Instance.AplicaComissao(percComissao, prod, orcamento);
                }
        ...
        public void AplicaAcrescimo(GDASession session, uint idProdutoOrcamento, int tipoAcrescimo, decimal acrescimo)
        {
            ...
            try
            {
                ...
                if (idItemProjeto == null)
                    produtos.AddRange(GetByProdutoOrcamento(session, idProdutoOrcamento));
                else
                    produtos.Add(GetElementByPrimaryKey(session, idProdutoOrcamento));
                
                ProdutosOrcamento produto = GetElementByPrimaryKey(session, idProdutoOrcamento);
                Orcamento orcamento = OrcamentoDAO.Instance.GetElementByPrimaryKey(session, produto.IdOrcamento);
                ...
                acrescimoAplicado = DescontoAcrescimo.Instance.AplicaAcrescimoAmbiente(tipoAcrescimo, acrescimo, produtos, orcamento);
        ...
        private void RemoveAcrescimo(GDASession session, uint idProdutoOrcamento, int tipoAcrescimo, decimal acrescimo)
        {
            ...
            try
            {
                ...
                Orcamento orcamento = produtos.Any()
                    ? OrcamentoDAO.Instance.GetElementByPrimaryKey(session, produtos[0].IdOrcamento)
                    : null;

                acrescimoRemovido = DescontoAcrescimo.Instance.RemoveAcrescimoAmbiente(produtos, orcamento);
        ...
        public void AplicaDesconto(GDASession session, uint idProdutoOrcamento, int tipoDesconto, decimal desconto)
        {
            ...
            try
            {
                ...
                descontoAplicado = DescontoAcrescimo.Instance.AplicaDescontoAmbiente(tipoDesconto, desconto, produtos, orcamento);
        ...
        private void RemoveDesconto(GDASession sessao, uint idProdutoOrcamento, int tipoDesconto, decimal desconto)
        {
            ...
            try
            {
                ...
                if (idItemProjeto == null)
                    produtos.AddRange(GetByProdutoOrcamento(sessao, idProdutoOrcamento));
                else
                    produtos.Add(GetElementByPrimaryKey(sessao, idProdutoOrcamento));

                Orcamento orcamento = produtos.Any()
                    ? OrcamentoDAO.Instance.GetElementByPrimaryKey(sessao, produtos[0].IdOrcamento)
                    : null;

                descontoRemovido = DescontoAcrescimo.Instance.RemoveDescontoAmbiente(produtos, orcamento);
        ...
        public void RecalcularValores(GDASession session, ProdutosOrcamento prod, uint? idCliente, int? tipoEntrega, bool somarAcrescimoDesconto)
        {
            ...
            try
            {
                ...
                var orcamento = OrcamentoDAO.Instance.GetElementByPrimaryKey(session, prod.IdOrcamento);
                ValorBruto.Instance.Calcular(prod, orcamento);
        ...
        public override uint Insert(GDASession sessao, ProdutosOrcamento objInsert)
        {
            ...
            try
            {
                ...
                if (objInsert.ValorProd > 0 && objInsert.Qtde > 0 && objInsert.Total == null || objInsert.Total == 0)
                {
                    ...
                    DescontoAcrescimo.Instance.AplicaComissao(orcamento.PercComissao, prod, orcamento);
        ...
        private void CalculaDescontoEValorBrutoProduto(GDASession session, ProdutosOrcamento produto, Orcamento orcamento = null)
        {
            ...
            DescontoAcrescimo.Instance.RemoveDescontoQtde(produto, orcamento);
            DescontoAcrescimo.Instance.AplicaDescontoQtde(produto, orcamento);
            DiferencaCliente.Instance.Calcular(produto, orcamento);
            ValorBruto.Instance.Calcular(produto, orcamento);
        }
