...
namespace Glass.Data.DAL
{
    public sealed class ProdutosPedidoEspelhoDAO : BaseDAO<ProdutosPedidoEspelho, ProdutosPedidoEspelhoDAO>
    {
        ...
        {
            DescontoAcrescimo.Calcular.Instance.AplicaAcrescimo(sessao, 2, valorAcrescimo, produtos, (int?)produtos[0].IdPedido, null, null);
        }
        ...
        {
            DescontoAcrescimo.Calcular.Instance.AplicaDesconto(sessao, 2, valorDesconto, produtos, (int?)produtos[0].IdPedido, null, null);
        }
        ...
        private void AplicaComissaoProdProj(GDASession sessao, uint idAmbientePedido, float percComissao, ProdutosPedidoEspelho[] produtos)
        {
            ...
            DescontoAcrescimo.Calcular.Instance.AplicaComissao(sessao, percComissao, produtos, (int?)produtos[0].IdPedido, null, null);
        }
            ...
            try
            {
                ...
                foreach (MaterialItemProjeto mip in MaterialItemProjetoDAO.Instance.GetByItemProjeto(sessao, itemProj.IdItemProjeto, false))
                {
                    ...
                    else if (!PedidoConfig.DadosPedido.AlterarValorUnitarioProduto)
                    {
                        ...
                        material.Valor = dadosObra != null && dadosObra.ProdutoValido ? dadosObra.ValorUnitProduto :
                            ProdutoDAO.Instance.GetValorTabela(sessao, (int)mip.IdProd, tipoEntrega, idCliente, false, itemProj.Reposicao, 0, (int)idPedidoEspelho, null, null);
                    ...
                    DescontoAcrescimo.Calcular.Instance.DiferencaCliente(sessao, prodPed, (int?)prodPed.IdPedido, null, null);
                ...
                foreach (ProdutosPedidoEspelho p in prodProj)
                    if (valorAcrescimoAplicado > 0 || valorDescontoAplicado > 0 || (percComissao > 0 && PedidoConfig.Comissao.ComissaoAlteraValor))
                        UpdateCompleto(sessao, p, false, atualizaReserva);
                    else
        ...
        public void RecalcularValores(ProdutosPedidoEspelho prodPed, uint idCliente, int tipoEntrega, bool somarAcrescimoDesconto)
        {
            ...
            try
            {
                ...
                DescontoAcrescimo.Calcular.Instance.CalculaValorBruto(null, prodPed);
                ...
                DescontoAcrescimo.Calcular.Instance.RecalcularValorUnit(null, prodPed, idCliente, tipoEntrega, !somarAcrescimoDesconto, benef.CountAreaMinima > 0, (int?)prodPed.IdPedido, null, null);
                ...
                if (prodPed.Total != valorTotal)
                    DescontoAcrescimo.Calcular.Instance.RecalcularValorUnit(prodPed, idCliente, tipoEntrega, (int?)prodPed.IdPedido, null, null);
        ...
        private void AplicarComissaoDescontoAcrescimo(GDASession session, int idPedido, ProdutosPedidoEspelho produtoPedidoEspelho)
        {
            ...
            var ambientesPedido = AmbientePedidoEspelhoDAO.Instance.GetByPedido(session, (uint)idPedido).Where(f => f.Acrescimo > 0).ToList();

            /* Chamado 62763. */
            foreach (var ambientePedido in ambientesPedido)
                AmbientePedidoEspelhoDAO.Instance.RemoveAcrescimo(session, ambientePedido.IdAmbientePedido);
            ...
            /* Chamado 62763. */
            foreach (var ambientePedido in ambientesPedido)
                AmbientePedidoEspelhoDAO.Instance.AplicaAcrescimo(session, ambientePedido.IdAmbientePedido, ambientePedido.TipoAcrescimo, ambientePedido.Acrescimo);
        }
