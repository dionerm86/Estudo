    ...
    public sealed class ProdutosPedidoEspelhoDAO : BaseDAO<ProdutosPedidoEspelho, ProdutosPedidoEspelhoDAO>
    {
            ...
            bool ignorarProdutoComposicao, out bool temFiltro, out string filtroAdicional)
        {
            ...
            string sql = "Select " + campos + @" From produtos_pedido_espelho pp 
                ...
                Left Join etiqueta_processo prc On (pp.idProcesso=prc.idProcesso) Where 1 " + FILTRO_ADICIONAL;
            ...
            float alturaMax, int larguraMin, int larguraMax, string idsRotas, int? fastDelivery, bool selecionar, int? idLoja)
        {
            string campos = selecionar ? @"pp.idProdPed, pp.idPedido, pp.idProd, pp.idItemProjeto, pp.idMaterItemProj, pp.idAmbientePedido, 
                ...
                pp.totM, pp.totM2Calc, pp.qtdImpresso, pp.total, pp.aliqIcms, pp.valorIcms, pp.ambiente, pp.obs, pp.obsGrid, pp.redondo, 
            ...
            string filtroApenasVidros = " And (p.idGrupoProd=" + (int)Glass.Data.Model.NomeGrupoProd.Vidro + @"
                    and (ped.tipoPedido=" + (int)Pedido.TipoPedidoEnum.Producao + " Or " + produtoParaVenda + "))";
            ...
            string sql = String.Empty;
                ...
                if (pecasRepostas != 2)
                {
                    ...
                    Where (pp.invisivelFluxo=false or pp.invisivelFluxo is null) 
                        ...
                        (maoDeObra == 2 ? " And ped.tipoPedido=" + (int)Pedido.TipoPedidoEnum.MaoDeObra : "" )+
                ...
                if (pecasRepostas == 1 || pecasRepostas == 2)
                {
                    ...
                    Where (invisivelFluxo=false or invisivelFluxo is null) and lp.idLeituraProd is null
                        ...
                        and (ped.tipoPedido=" + (maoDeObra == 2 ? (int)Pedido.TipoPedidoEnum.MaoDeObra : (int)Pedido.TipoPedidoEnum.Producao) + " Or " + produtoParaVenda + "))" + where + " group by pp.idProdPed";
                }
            ...
            if (maoDeObra == 1)
            {
                string camposUnion = selecionar ? @"pp.idProdPed, pp.idPedido, ape.idProd, pp.idItemProjeto, pp.idMaterItemProj, 
                    ...
                    pp.totM, pp.totM2Calc, ape.qtdeImpresso, pp.total, pp.aliqIcms, pp.valorIcms, ape.ambiente, pp.obs, pp.obsGrid, 
        ...
        public IList<ProdutosPedidoEspelho> GetProdToEtiq(uint idPedido, uint idProcesso, uint idAplicacao, uint idCorVidro,
            float espessura, uint idSubgrupoProd, float alturaMin, float alturaMax, int larguraMin, int larguraMax)
        {
            ...
            return objPersistence.LoadData(SqlProdEtiq(idPedido, 0, null, idCorVidro, espessura, 1, null, null, null, null, 
                ...
                larguraMax, null, true), GetParam(null, null, null, null, null, codProc, codApl)).ToList();
        }
        ...
        public bool PossuiPecaASerImpressa(GDASession sessao, uint idPedido)
        {
            ...
            {
                int situacaoProducao = PedidoDAO.Instance.ObtemSituacaoProducao(sessao, idPedido);
                if (situacaoProducao > (int)Pedido.SituacaoProducaoEnum.Pendente)
                    return false;
            }
        ...
        public IList<ProdutosPedidoEspelho> GetImprPendRpt(uint idPedido, uint idCorVidro, int espessura, string dataIni, string dataFim, bool buscarPecasDeBox,
            bool buscarPecasRepostas, uint idRota, string codProcesso, string codAplicacao, int? idLoja)
        {
            int? pecasRepostas = buscarPecasRepostas ? 1 : (int?)null;

            return objPersistence.LoadData(SqlProdEtiq(idPedido, 0, null, idCorVidro, espessura, 1, dataIni, dataFim, null, null, codProcesso, codAplicacao,
                buscarPecasDeBox, pecasRepostas, idRota, false, 0, 0, 0, 0, 0, null, null, true, idLoja) /*+ " Order by ped.dataEntrega, ped.idPedido"*/,
                GetParam(null, dataIni, dataFim, null, null, codProcesso, codAplicacao)).ToList();
        }
            ...
            bool buscarPecasRepostas, uint idRota, string codProcesso, string codAplicacao, int? idLoja, string sortExpression, int startRow, int pageSize)
        {
            ...
            return LoadDataWithSortExpression(SqlProdEtiq(idPedido, 0, null, idCorVidro, espessura, 1, dataIni, dataFim, null, null, codProcesso, codAplicacao,
                buscarPecasDeBox, pecasRepostas, idRota, false, 0, 0, 0, 0, 0, null, null, true, idLoja), sort, startRow, pageSize, 
            ...
            int alturaIni, int alturaFim, int larguraIni, int larguraFim, string codProcesso, string codAplicacao, bool selecionar)
        {
            ...
            string campos = selecionar ? @"prod.idProdPed, prod.idProdNf, prod.idPedido, prod.numeroNFe, prod.idProd,
                ...
                null as valorIcms, null as aliquotaIpi, null as valorIpi, null as ambiente, null as obs, null as obsGrid,
        ...
        public void MarcarImpressao(GDASession session, uint idProdPed, int qtdImpresso, string obs)
        {
        ...
        public ProdutosPedidoEspelho[] GetForRptPcp(string idPedidos, string grupos, string produtos, bool agruparProduto)
        {
            ...
            if (PedidoConfig.DadosPedido.AmbientePedido && (!String.IsNullOrEmpty(produtos) || PCPConfig.RelatorioPedidoPcp.SempreAgruparProdutosPorProdutoAlturaLargura))
            ...
            {
                if (PCPConfig.ControlarProducao && !PCPConfig.ExibirEtiquetasImpressaoPcp && 
                    !PedidoDAO.Instance.IsProducao(ppe.IdPedido) &&
                    ProdutoDAO.Instance.IsVidro(null, (int)ppe.IdProd))
                    ppe.Etiquetas = PecaItemProjetoDAO.Instance.ObtemEtiquetas(ppe.IdPedido, ppe.IdProdPed, (int)ppe.Qtde);
            ...
            int situacao, string situacaoPedOri, string dataIniFin, string dataFimFin, string idsRota, bool selecionar)
        {
            var campos = selecionar ? @"pp.idPedido, Cast((ppeb.qtd * pp.qtde) As Signed) As Qtde, Cast((pp.altura + bc.acrescimoAltura) As Signed) As Altura,
                ...
                pp.obsGrid, pp.redondo, pp.valorBenef, pp.pedCli, pp.alturaBenef, pp.larguraBenef, pp.espBenef, pp.valorAcrescimo, pp.valorDesconto,
                pp.valorAcrescimoProd, pp.valorDescontoProd, pp.percDescontoQtde, pp.valorDescontoQtde, pp.valorDescontoCliente,
        ...
        public string ObtemForma(GDASession session, uint idProdPed)
        {
            ...
            if (String.IsNullOrEmpty(forma))
                forma = ProdutoDAO.Instance.ObtemForma(session, (int)ObtemIdProd(session, idProdPed));
        ...
        {
            return ObtemValorCampo<string>(session, "Concat(Coalesce(obsGrid, ''), ' ', Coalesce(obs, ''))", "idProdPed=" + idProdPed);
        }
        ...
        {
            return ObtemValorCampo<int>("QtdImpresso", "IdProdPed="+ idProdPed);
        }
        ...
        {
            DescontoAcrescimo.Instance.AplicaAcrescimo(sessao, 2, valorAcrescimo, produtos);
        }
        ...
        {
            DescontoAcrescimo.Instance.AplicaDesconto(sessao, 2, valorDesconto, produtos);
        }
        ...
        private void AplicaComissaoProdProj(GDASession sessao, uint idAmbientePedido, float percComissao, ProdutosPedidoEspelho[] produtos)
        {
            ...
            DescontoAcrescimo.Instance.AplicaComissao(sessao, percComissao, produtos);
        }
        ...
        /// Insere/Atualiza Produto de Projeto
        /// </summary>
        public uint InsereAtualizaProdProj(GDASession sessao, uint idPedidoEspelho, uint? idAmbientePedidoEsp, ItemProjeto itemProj,
            bool medidasAlteradas)
        {
            ...
            float percComissao = PedidoEspelhoDAO.Instance.ObtemValorCampo<float>(sessao, "percComissao", "idPedido=" + idPedidoEspelho);

            return InsereAtualizaProdProj(sessao, idPedidoEspelho, idAmbientePedidoEsp, itemProj, valorAcrescimoAplicado, 
                valorDescontoAplicado, percComissao, true, medidasAlteradas);
        }
        ...
        {
            return InsereAtualizaProdProj(sessao, idPedidoEspelho, idAmbientePedidoEsp, itemProj, valorAcrescimoAplicado,
                valorDescontoAplicado, percComissao, atualizaReserva, medidasAlteradas, null);
        }
            ...
            Dictionary<int, int> associacaoProdutosPedidoProdutosPedidoEspelho)
        {
            try
            {
                ...
                ambienteEspelho.IdPedido = idPedidoEspelho;
                ...
                foreach (ProdutosPedidoEspelho ppe in GetByAmbienteFast(sessao, idPedidoEspelho, idAmbientePedidoEsp.Value))
                {
                ...
                foreach (MaterialItemProjeto mip in MaterialItemProjetoDAO.Instance.GetByItemProjeto(sessao, itemProj.IdItemProjeto, false))
                {
                    ...
                    //Chamado 49030
                    else if (!PedidoConfig.DadosPedido.AlterarValorUnitarioProduto)
                    {
                        ...
                        material.Valor = dadosObra != null && dadosObra.ProdutoValido ? dadosObra.ValorUnitProduto :
                            ProdutoDAO.Instance.GetValorTabela(sessao, (int)mip.IdProd, tipoEntrega, idCliente, false, itemProj.Reposicao, 0);
                        ...
                        mip.Valor = material.Valor;
                        mip.TotM = material.TotM;
                        mip.Total = material.Total;
                        mip.Custo = material.Custo;
                        mip.TotM2Calc = material.TotM2Calc;
                    }

                    ProdutosPedidoEspelho prodPed = new ProdutosPedidoEspelho();
                    prodPed.IdPedido = idPedidoEspelho;
                    ...
                    if (Glass.Data.DAL.GrupoProdDAO.Instance.IsAluminio(ProdutoDAO.Instance.ObtemIdGrupoProd(sessao, (int)prodPed.IdProd)) &&
                        mip.AlturaCalc > 0)
                    ...
                    DescontoAcrescimo.Instance.DiferencaCliente(sessao, prodPed);
                    prodPed.IdProdPed = InsertFromProjeto(sessao, prodPed);

                    /* Chamado 50709. */
                    if (associacaoProdutosPedidoProdutosPedidoEspelho != null)
                    {
                        var idProdPedIdMaterItemProj = (int)ProdutosPedidoDAO.Instance.GetIdProdPedByMaterItemProj(sessao, idPedidoEspelho, mip.IdMaterItemProjOrig.GetValueOrDefault());
                ...
                if (valorAcrescimoAplicado > 0)
                    AplicaAcrescimoProdProj(sessao, idAmbientePedidoEsp.Value, valorAcrescimoAplicado, prodProj);

                if (valorDescontoAplicado > 0)
                    AplicaDescontoProdProj(sessao, idAmbientePedidoEsp.Value, valorDescontoAplicado, prodProj);

                if (percComissao > 0)
                    AplicaComissaoProdProj(sessao, idAmbientePedidoEsp.Value, percComissao, prodProj);

                // Atualiza os produtos
                foreach (ProdutosPedidoEspelho p in prodProj)
                    if (valorAcrescimoAplicado > 0 || valorDescontoAplicado > 0 || (percComissao > 0 &&
                        PedidoConfig.Comissao.ComissaoAlteraValor))
                    {
                        p.AtualizarTotalPedido = false;
                        ProdutosPedidoEspelhoDAO.Instance.UpdateCompleto(sessao, p, false, atualizaReserva);
                    }
                    else
                ...
                if (itemProj.IdPedidoEspelho == null && itemProj.IdPedido == null)
                    objPersistence.ExecuteCommand(sessao, "Update item_projeto Set idPedidoEspelho=" + idPedidoEspelho + " Where idItemProjeto=" + itemProj.IdItemProjeto);

                // Atualiza o total do pedido
                PedidoEspelhoDAO.Instance.UpdateTotalPedido(sessao, idPedidoEspelho, true);
                ...
                if (OrcamentoConfig.Desconto.DescontoAcrescimoItensOrcamento)
                {
                    if (ambienteEspelho.Acrescimo > 0)
                        AmbientePedidoEspelhoDAO.Instance.AplicaAcrescimo(sessao, ambienteEspelho.IdAmbientePedido, ambienteEspelho.TipoAcrescimo, ambienteEspelho.Acrescimo);

                    if (ambienteEspelho.Desconto > 0)
                        AmbientePedidoEspelhoDAO.Instance.AplicaDesconto(sessao, ambienteEspelho.IdAmbientePedido, ambienteEspelho.TipoDesconto, ambienteEspelho.Desconto);
                }
        ...
        public void AtualizarEdicaoImagemPecaArquivoMarcacao(int idProdPedAtual, int? idProdPedNovo, bool medidasAlteradas)
        {
            ...
            {
                if (idProdPedNovo > 0 && !medidasAlteradas && ProjetoConfig.ManterImagensEditadasAoConfirmarProjeto)
        ...
        /// <param name="idProdPed"></param>
        /// <returns></returns>
        public new bool Exists(uint idProdPed)
        {
        ...
        /// </summary>
        /// <param name="idProd"></param>
        public void RecalcularValores(uint idProdPed, bool somarAcrescimoDesconto)
        {
            ProdutosPedidoEspelho prod = GetElementByPrimaryKey(idProdPed);
            RecalcularValores(prod, somarAcrescimoDesconto);
            Update(prod);
        }
        ...
        public void RecalcularValores(ProdutosPedidoEspelho prodPed, bool somarAcrescimoDesconto)
        {
            uint idCliente = PedidoDAO.Instance.ObtemIdCliente(prodPed.IdPedido);
            int tipoEntrega = PedidoDAO.Instance.ObtemTipoEntrega(prodPed.IdPedido);

            RecalcularValores(prodPed, idCliente, tipoEntrega, somarAcrescimoDesconto);
        }
        ...
        /// </summary>
        /// <param name="prod"></param>
        public void RecalcularValores(ProdutosPedidoEspelho prodPed, uint idCliente, int tipoEntrega, bool somarAcrescimoDesconto)
        {
            ...
            try
            {
                ...
                DescontoAcrescimo.Instance.CalculaValorBruto(null, prodPed);
                ...
                DescontoAcrescimo.Instance.RecalcularValorUnit(null, prodPed, idCliente, tipoEntrega, !somarAcrescimoDesconto, benef.CountAreaMinima > 0);

                Glass.Data.DAL.ProdutoDAO.Instance.CalcTotaisItemProd(idCliente, (int)prodPed.IdProd, prodPed.Largura, prodPed.Qtde, prodPed.QtdeAmbiente, prodPed.ValorVendido, prodPed.Espessura, prodPed.Redondo,
                    2, false, true, ref custo, ref altura, ref totM2, ref totM2Calc, ref valorTotal, false, prodPed.Beneficiamentos.CountAreaMinima, true);
                ...
                if (prodPed.Total != valorTotal)
                    DescontoAcrescimo.Instance.RecalcularValorUnit(prodPed, idCliente, tipoEntrega);
        ...
        public bool PossuiFml(GDASession sessao, uint idProdPed, string etiqueta, bool paraDestaqueEtiqueta)
        {
            ...
            if (pecaProjMod == null && PCPConfig.EmpresaGeraArquivoFml)
            {
                ...
                if (pedidoImportado)
                {
                    var nomeArquivo = ImpressaoEtiquetaDAO.Instance.ObtemNomeArquivo((int)idProdPed, etiqueta, TipoArquivoMesaCorte.FML, false);
        ...
        public bool PossuiDxf(GDASession sessao, uint idProdPed, string etiqueta)
        {
            ...
            if (pecaProjMod == null && PCPConfig.EmpresaGeraArquivoDxf)
            {
                ...
                if (pedidoImportado)
                {
                    var nomeArquivo = ImpressaoEtiquetaDAO.Instance.ObtemNomeArquivo((int)idProdPed, etiqueta, TipoArquivoMesaCorte.DXF, false);
        ...
        public bool PossuiSGlass(GDASession sessao, uint idProdPed, string etiqueta)
        {
            ...
            using (Glass.Seguranca.AutenticacaoRemota.Autenticar())
            {
                try
                {
                    var nomeArquivoDxf = ImpressaoEtiquetaDAO.Instance.ObtemNomeArquivo(sessao, (int)idProdPed, etiqueta, TipoArquivoMesaCorte.DXF, false);
        ...
        internal void RemoverClone(GDASession sessao, uint idProdPed)
        {
            ...
            string idsProdPed = String.Join(",",
            ExecuteMultipleScalar<string>(sessao, "select idProdPed from produtos_pedido_espelho where idPedido=" + idPedido).ToArray());
        ...
        public uint InsertFromProjeto(GDASession sessao, ProdutosPedidoEspelho objInsert)
        {
            uint idProdPedEsp = base.Insert(sessao, objInsert);

            AtualizaBenef(sessao, idProdPedEsp, objInsert.Beneficiamentos);

            // Cria uma cópia do produto na tabela ProdutosPedido, com referência ao produto que está sendo inserido e invisível ao pedido
            objInsert.IdProdPed = idProdPedEsp;

            if (objInsert.Espessura == 0 && ProdutoDAO.Instance.ObtemIdGrupoProd((int)objInsert.IdProd) == (int)Glass.Data.Model.NomeGrupoProd.Vidro)
                objInsert.Espessura = ProdutoDAO.Instance.ObtemEspessura((int)objInsert.IdProd);
        ...
        public override uint Insert(GDASession session, ProdutosPedidoEspelho objInsert)
        {
            ...
            try
            {
                ...
                if (FinanceiroConfig.UsarControleDescontoFormaPagamentoDadosProduto)
                {
                    ...
                    if(produtoPedidoEspelhoInserido.Count > 0)
                    {
                        if (produtoPedidoEspelhoInserido[0].IdGrupoProd != objInsert.IdGrupoProd)
                        ...
                        if (produtoPedidoEspelhoInserido[0].IdSubgrupoProd != objInsert.IdSubgrupoProd)
                ...
                if (ProdutoDAO.Instance.IsVidro(session, (int)objInsert.IdProd))
                {
                    ...
                    if (idSubgrupoProd > 0)
                        if (SubgrupoProdDAO.Instance.ObtemTipoCalculo(session, idSubgrupoProd.Value, false) == (int)Glass.Data.Model.TipoCalculoGrupoProd.Qtd)
                        {
                ...
                int qtdeAmbiente = PedidoDAO.Instance.IsMaoDeObra(session, objInsert.IdPedido) ? AmbientePedidoEspelhoDAO.Instance.GetQtde(session, objInsert.IdAmbientePedido) : 1;
                ...
                float totM2 = objInsert.TotM, altura = objInsert.Altura, totM2Calc = objInsert.TotM2Calc;
                decimal total = objInsert.Total, custoProd = 0;
                bool redondo = objInsert.Redondo || (objInsert.IdAmbientePedido > 0 ? AmbientePedidoEspelhoDAO.Instance.IsRedondo(session, objInsert.IdAmbientePedido.Value) : false);
                var isPedidoProducaoCorte = PedidoDAO.Instance.IsPedidoProducaoCorte(session, objInsert.IdPedido);

                Glass.Data.DAL.ProdutoDAO.Instance.CalcTotaisItemProd(session, idCliente, (int)objInsert.IdProd, objInsert.Largura, qtde, qtdeAmbiente, objInsert.ValorVendido,
                    objInsert.Espessura, redondo, 0, false, !isPedidoProducaoCorte, ref custoProd, ref altura, ref totM2, ref totM2Calc, ref total, alturaBenef,
                    larguraBenef, false, objInsert.Beneficiamentos.CountAreaMinimaSession(session), true);

                objInsert.TotM = totM2;
                objInsert.Total = total;
                objInsert.TotM2Calc = totM2Calc;

                if (ProdutoDAO.Instance.ObtemIdGrupoProd(session, (int)objInsert.IdProd) == (int)Glass.Data.Model.NomeGrupoProd.Vidro && objInsert.Espessura == 0)
                ...
                try
                {
                    AtualizaBenef(session, returnValue, objInsert.Beneficiamentos);
                }
                catch { }
                ...
                if (FinanceiroConfig.UsarControleDescontoFormaPagamentoDadosProduto)
                {
                    var pedidoEspelho = PedidoEspelhoDAO.Instance.GetElement(session, objInsert.IdPedido);
                    var tipoVenda = PedidoDAO.Instance.ObtemTipoVenda(session, pedidoEspelho.IdPedido);
                    var idFormaPagto = PedidoDAO.Instance.ObtemFormaPagto(session, pedidoEspelho.IdPedido);
                    var idTipoCartao = PedidoDAO.Instance.ObtemTipoCartao(session, pedidoEspelho.IdPedido);
                    var idParcela = PedidoDAO.Instance.ObtemIdParcela(session, pedidoEspelho.IdPedido);
                    var idGrupoProd = objInsert.IdGrupoProd;
                    var idSubgrupoProd = objInsert.IdSubgrupoProd;
                    var desconto = DescontoFormaPagamentoDadosProdutoDAO.Instance.ObterDesconto(session, (uint)tipoVenda, idFormaPagto, idTipoCartao, idParcela, idGrupoProd, idSubgrupoProd);

                    pedidoEspelho.Desconto = desconto;
                    PedidoEspelhoDAO.Instance.UpdateDados(session, pedidoEspelho);
                }

                // Cria uma cópia do produto na tabela ProdutosPedido, com referência ao produto que está sendo inserido e invisível ao pedido
                objInsert.IdProdPed = returnValue;
                CriarClone(session, objInsert, true, false);
            }
            catch (Exception ex)
            {
        ...
        private void AplicarComissaoDescontoAcrescimo(GDASession session, int idPedido, ProdutosPedidoEspelho produtoPedidoEspelho)
        {
            var percentualComissao = PedidoEspelhoDAO.Instance.ObterPercentualComissao(session, idPedido);
            var tipoAcrescimo = PedidoEspelhoDAO.Instance.ObterTipoAcrescimo(session, idPedido);
            var acrescimo = PedidoEspelhoDAO.Instance.ObterAcrescimo(session, idPedido);
            var tipoDesconto = PedidoEspelhoDAO.Instance.ObterTipoDesconto(session, idPedido);
            var desconto = PedidoEspelhoDAO.Instance.ObterDesconto(session, idPedido);

            if (percentualComissao > 0)
                PedidoEspelhoDAO.Instance.RemoveComissao(session, (uint)idPedido, produtoPedidoEspelho);

            if (acrescimo > 0)
                PedidoEspelhoDAO.Instance.RemoveAcrescimo(session, (uint)idPedido, produtoPedidoEspelho);

            if (desconto > 0)
                PedidoEspelhoDAO.Instance.RemoveDesconto(session, (uint)idPedido, produtoPedidoEspelho);

            if (percentualComissao > 0)
                PedidoEspelhoDAO.Instance.AplicaComissao(session, (uint)idPedido, percentualComissao, produtoPedidoEspelho);

            if (acrescimo > 0)
                PedidoEspelhoDAO.Instance.AplicaAcrescimo(session, (uint)idPedido, tipoAcrescimo, acrescimo, produtoPedidoEspelho);

            if (desconto > 0)
                PedidoEspelhoDAO.Instance.AplicaDesconto(session, (uint)idPedido, tipoDesconto, desconto, produtoPedidoEspelho);
        }

        internal int UpdateBase(ProdutosPedidoEspelho objUpdate)
        {
        ...
        {
            return UpdateCompleto(sessao, objUpdate, true, true);
        }

        public int UpdateCompleto(GDASession sessao, ProdutosPedidoEspelho objUpdate, bool removerClone, bool atualizaReserva)
        {
            try
            {
                ...
                int qtdeAmbiente = PedidoDAO.Instance.IsMaoDeObra(sessao, objUpdate.IdPedido) ?
                    AmbientePedidoEspelhoDAO.Instance.GetQtde(sessao, objUpdate.IdAmbientePedido) : 1;
                ...
                float totM2 = objUpdate.TotM, altura = objUpdate.Altura, totM2Calc = objUpdate.TotM2Calc;
                bool redondo = objUpdate.Redondo || (objUpdate.IdAmbientePedido > 0 ? AmbientePedidoEspelhoDAO.Instance.IsRedondo(sessao, objUpdate.IdAmbientePedido.Value) : false);
                ...
                Glass.Data.DAL.ProdutoDAO.Instance.CalcTotaisItemProd(sessao, idCliente, (int)objUpdate.IdProd, objUpdate.Largura, qtde, qtdeAmbiente,
                    objUpdate.ValorVendido, objUpdate.Espessura, redondo, 0, false, !isPedidoProducaoCorte, ref custoProd, ref altura, ref totM2, ref totM2Calc, ref total,
                    alturaBenef, larguraBenef, false, objUpdate.Beneficiamentos.CountAreaMinimaSession(sessao), true);
                ...
                /* Chamado 28687. */
                if (PedidoDAO.Instance.IsPedidoImportado(objUpdate.IdPedido) && objUpdate.Obs == null)
                    objUpdate.Obs = ObtemObs(objUpdate.IdProdPed);
                ...
                if (removerClone)
                    RemoverClone(sessao, objUpdate.IdProdPed);

                CriarClone(sessao, objUpdate, atualizaReserva, false);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (objUpdate.AtualizarTotalPedido)
                    PedidoEspelhoDAO.Instance.UpdateTotalPedido(sessao, objUpdate.IdPedido, true);
            }
            catch (Exception ex)
            {
