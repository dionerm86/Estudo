...
namespace Glass.Global.Negocios.Componentes
{
        ...
        Entidades.IProvedorFuncionario
    {
        ...
        /// <returns></returns>
        public IList<Entidades.Departamento> PesquisarDepartamentos(int? idDepartamento, string nome)
        {
            var consulta = SourceContext.Instance.CreateQuery()
                .From<Data.Model.Departamento>()
                .OrderBy("Nome");

            if (idDepartamento.HasValue && idDepartamento.Value > 0)
                consulta.WhereClause
                    .And("IdDepartamento = ?idDepartamento")
                    .Add("?idDepartamento", idDepartamento.Value);
            ...
            return consulta.ToVirtualResultLazy<Entidades.Departamento>();
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IList<Colosoft.IEntityDescriptor> ObteDepartamentos()
        {
            return SourceContext.Instance.CreateQuery()
                .From<Data.Model.Departamento>()
                .OrderBy("Nome")
                .ProcessResultDescriptor<Entidades.Departamento>()
                .ToList();
        }
        ...
        /// <returns></returns>
        public Entidades.Departamento ObtemDepartamento(int idDepartamento)
        {
            return SourceContext.Instance.CreateQuery()
                .From<Data.Model.Departamento>()
                .Where("IdDepartamento=?id")
                .Add("?id", idDepartamento)
                .ProcessLazyResult<Entidades.Departamento>()
                .FirstOrDefault();
        }
        ...
        /// <returns></returns>
        public Colosoft.Business.SaveResult SalvarDepartamento(Entidades.Departamento departamento)
        {
            departamento.Require("departamento").NotNull();

            using (var session = SourceContext.Instance.CreateSession())
            {
                var resultado = departamento.Save(session);
                if (!resultado)
                    return resultado;
        ...
        /// <param name="departamento"></param>
        /// <returns></returns>
        public Colosoft.Business.DeleteResult ApagarDepartamento(Entidades.Departamento departamento)
        {
            departamento.Require("departamento").NotNull();

            using (var session = SourceContext.Instance.CreateSession())
            {
                var resultado = departamento.Delete(session);
                if (!resultado)
                    return resultado;
        ...
        /// <param name="departamento"></param>
        /// <returns></returns>
        IMessageFormattable[] Entidades.IValidadorDepartamento.ValidaExistencia(Entidades.Departamento departamento)
        {
            if (SourceContext.Instance.CreateQuery()
                .From<Data.Model.FuncDepartamento>()
                .Where("IdDepartamento=?id")
                .Add("?id", departamento.IdDepartamento)
                .ExistsResult())
                return new IMessageFormattable[]
                {
                    "Há funcionário(s) associado(s) ao mesmo.".GetFormatter()
                };
        ...
        /// <returns></returns>
        public IList<Colosoft.IEntityDescriptor> ObtemFuncionariosPorDepartamento(int idDepartamento)
        {
            return SourceContext.Instance.CreateQuery()
                .From<Data.Model.Funcionario>("f")
                .InnerJoin<Data.Model.FuncDepartamento>("f.IdFunc = fd.IdFunc", "fd")
                .Where("fd.IdDepartamento = ?idDepartamento")
                .Add("?idDepartamento", idDepartamento)
                .ProcessResultDescriptor<Entidades.Funcionario>()
                .ToList();
        }
        ...
        /// <param name="idDepartamento">Identificador do departamento.</param>
        /// <returns></returns>
        public IList<Colosoft.IEntityDescriptor> ObtemFuncionariosForaDepartamento(int idDepartamento)
        {
            return SourceContext.Instance.CreateQuery()
                .From<Data.Model.Funcionario>("f")
                .LeftJoin<Data.Model.FuncDepartamento>("f.IdFunc = fd.IdFunc AND fd.IdDepartamento = ?idDepartamento", "fd")
                .Where("fd.IdDepartamento IS NULL")
                .Add("?idDepartamento", idDepartamento)
                .ProcessResultDescriptor<Entidades.Funcionario>()
                .ToList();
        }
        ...
        /// <param name="idDepartamento">Identificador do departamento.</param>
        /// <returns></returns>
        public Colosoft.Business.SaveResult AdicionarFuncionarioDepartamento(int idFunc, int idDepartamento)
        {
            using (var session = SourceContext.Instance.CreateSession())
            {
                var funcDepartamento = new Entidades.FuncDepartamento
                {
                    IdFunc = idFunc,
                    IdDepartamento = idDepartamento
                };

                var resultado = funcDepartamento.Save(session);
                if (!resultado)
                    return resultado;
        ...
        /// <param name="idDepartamento">Identificador do departamento.</param>
        /// <returns></returns>
        public Colosoft.Business.DeleteResult ApagarFuncionarioDepartamento(int idFunc, int idDepartamento)
        {
            using (var session = SourceContext.Instance.CreateSession())
            {
                var funcDepartamento = new Entidades.FuncDepartamento
                ...
                funcDepartamento.DataModel.ExistsInStorage = true;

                var resultado = funcDepartamento.Delete(session);
                if (!resultado)
                    return resultado;
