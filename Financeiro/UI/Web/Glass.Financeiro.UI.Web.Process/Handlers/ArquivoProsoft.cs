...
{
    /// <summary>
    /// Summary description for ArquivoProsoft
    /// </summary>
    public class ArquivoProsoft : IHttpHandler
    {
        #region Propiedades
        ...
        public void ProcessRequest(HttpContext context)
        {
            try
            {
                ...
                var prosoftFluxo = ServiceLocator.Current.GetInstance<Glass.Financeiro.Negocios.IProsoftFluxo>();

                var arq = new Glass.Financeiro.Negocios.Entidades.Prosoft.Arquivo();

                if (Receber)
                    arq = prosoftFluxo.GerarArquivoRecebidas(IdPedido, IdLiberarPeidido, IdAcerto, IdAcertoParcial, IdTrocaDev, NumeroNfe,
                    ...
                    arq = prosoftFluxo.GerarArquivoPagas(IdContaPg, IdCompra, NumeroNfe, IdCustoFixo, IdImpServ, IdComissao, ValorInicial, ValorFinal, DtInivenc, DtFimVenc, DtIniRec, DtFimRec,
                        IdLoja, IdFornec, NomeFornec, IdFormaPagto, IdConta, JurosMulta, Observacao);

                if (arq == null)
                    throw new Exception("Nenhuma conta encontrada.");

                var data = DateTime.Now;
                var nomeArquivo = "PROSOFT_" + data.Day + "_" + data.Month + "_" + data.Year + "_" + data.Millisecond + ".txt";
