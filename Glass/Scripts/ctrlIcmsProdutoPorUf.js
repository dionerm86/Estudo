/**
 * Classe com os dados para o controle de ICMS de produto por UF.
 * @param nomeControle O nome do controle que está sendo criado.
 */
var IcmsProdutoPorUfType = function(nomeControle)
{
    ...
    recuperaControlesItem = function(numeroItem)
    {
        ...
        if (!_itens[numeroItem])
        {
            ...
            var item = {
                ...
                "Labels": [
                    FindControl("lblTipoCliente", "span", item),
     ...
     * @param idTipoCliente O tipo de cliente da linha que está sendo alterada.
     * @param ufOrigem A UF de origem da linha que está sendo alterada.
     */
    bloqueiaUfSelecionada = function(numeroLinha, idTipoCliente, ufOrigem)
    {
    ...
    this.ValidaItemExcecao = function(val, args)
    {
        ...
        for (var i = 0; i < controles.Campos.length; i++)
            ...
            // Permite que o campo de tipo de cliente fique vazio
            else if (controles.Campos[i].id.indexOf("drpTipoCliente") == -1)
            {
                vazio = true;
                if (preenchido) break;
            }
    ...
    ValidatorHookupControl(FindControl("txtIcmsIntra", "input", geral), FindControl("ctvGeral", "span", geral));
