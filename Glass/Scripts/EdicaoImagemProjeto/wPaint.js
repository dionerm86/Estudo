...
* @link            www.websanova.com
* @github			    github.com/websanova/wPaint
...
(function($) {
    ...
    var shapes = ['Rectangle', 'Ellipse', 'Line', 'Text', 'Text_V'];
    ...
    $.fn.wPaint.defaultSettings = {
        ...
        fontSize: '13', 			// current font size for text input
        fontFamilyOptions: ['Arial', 'Courier', 'Times', 'Trebuchet', 'Verdana'], // available font families
        fontFamily: 'Arial', 		// active font family for text input
        fontTypeBold: true, 		// text input bold enable/disable
        ...
        menu: ['open', 'save', 'undo', 'clear', 'resize', 'rectangle', 'ellipse', 'line', 'pencil', 'text', 'text_v', 'eraser', 'lineWidth', 'fillColor', 'strokeColor', 'biblioteca_imagens', 'imagem_padrao'], // menu items - appear in order they are set
    ...
    Canvas.prototype =
	{
	    ...
	    *******************************************************************************/
	    generate: function(width, height) {
	        ...
	        // Eventos para teclado para o editor ==============================
	        $(document).keydown(function(e) {
	            if (e.ctrlKey && e.keyCode == 90) // CTRL + Z - Desfazer
	            {
	                _self.undoPrev();
	            }
	        });
	    ...
	    generateTextInput: function() {
			...
			.css({ display: 'none', position: 'absolute', color: this.settings.strokeStyle, fontSize: this.settings.fontSize, lineHeight: this.settings.fontSize + 'px', fontFamily: this.settings.fontFamily })
	    ...
	    callFunc: function(e, _self, event) {
	        ...
	        if (_self.settings.mode !== 'Text' && _self.settings.mode !== 'Text_V' && event === 'Up') { this.addUndo(); }
	    },
	    ...
	    *******************************************************************************/
	    drawShapeDown: function(e, _self) {
	        if (_self.settings.mode == 'Text' || _self.settings.mode == 'Text_V') {
	    ...
	    drawShapeMove: function(e, _self) {
	        ...
	        if (_self.settings.mode == 'Text' || _self.settings.mode == 'Text_V') _self.textInput.css({ left: left - 1, top: top - 1, width: width, height: height });
	    ...
	    *******************************************************************************/
	    drawPencilDown: function(e, _self) {
	        ...
	        if ($("._wPaint_eraser").hasClass('active') == true) {
	            _self.ctx.lineWidth = 10;
	            _self.ctx.strokeStyle = "#FFFFFF";
	        }
	        else {
	        ...
	        _self.ctx.beginPath();
	        if ($("._wPaint_eraser").hasClass('active') == true) {
	            _self.ctx.arc(e.pageX + 5, e.pageY + 5, _self.settings.lineWidth / 2, 0, Math.PI * 2, true);
	        }
	        else if ($("._wPaint_pencil").hasClass('active') == true) {
	            _self.ctx.arc(e.pageX + 1, e.pageY + 14, _self.settings.lineWidth / 2, 0, Math.PI * 2, true);
	        }
	        else {
	        ...
	        _self.ctx.beginPath();
	        if ($("._wPaint_eraser").hasClass('active') == true) {
	            _self.ctx.moveTo(e.pageX + 5, e.pageY + 5);
	        }
	        else if ($("._wPaint_pencil").hasClass('active') == true) {
	            _self.ctx.moveTo(e.pageX + 1, e.pageY + 14);
	        }
	        else {
	    ...
	    drawPencilMove: function(e, _self) {
	        if ($("._wPaint_eraser").hasClass('active') == true) {
	            _self.ctx.lineTo(e.pageX + 5, e.pageY + 5);
	        }
	        else if ($("._wPaint_pencil").hasClass('active') == true) {
	            _self.ctx.lineTo(e.pageX + 1, e.pageY + 14);
	        }
	        else {
	    ...
	    *******************************************************************************/
	    ...
	    drawTextDown: function(e, _self) {
	        _self.textInput.css({ height: '22px', width: '130px' });
	        _self.textInput.val('').show().focus();

	        $("._wPaint_textInput").moveable({ img: $('#hidden_contexto').val() + "/images/EdicaoImagemProjeto/arrow.png" });

	        $("#img_change_position_text").hide();

	        var left = $("._wPaint_textInput").position().left;
	        var top = $("._wPaint_textInput").position().top;
	        $("#img_change_position_text").css('left', left - 15);
	        $("#img_change_position_text").css('top', top - 10);

	        $("#img_change_position_text").show();
	    },
	    ...
	    drawTextUp: function(e, _self) {
	        ...
	        var altura_entrada_texto = _self.textInput.height();
	        var largura_entrada_texto = _self.textInput.width();
	        ...
	        for (var i = 0, ii = lines.length; i < ii; i++) {
	            ...
	            if ($('#hidden_tipo_texto_selecionado').val() == 'V') {
	                _self.ctx.translate((left + _self.settings.fontSize * i) + 2, top + largura_entrada_texto);
	                _self.ctx.rotate(-0.5 * Math.PI);
	                _self.ctx.fillText(lines[i], 0, 0);
	            }
	            else {
	                _self.ctx.fillText(lines[i], left + 2, top + 2);
    ...
    */
    ...
    MainMenu.prototype =
	{
	    generate: function(canvas, textMenu) {
	        ...
	        var lineWidth = $('<div class="_wPaint_lineWidth _wPaint_dropDown" title="Espessura"></div>').append(
	        ...
	        $.each($canvas.settings.menu, function(i, item) {
	            switch (item) {
	                case 'undo': menuContent.append($('<div id="div_undo" class="_wPaint_icon _wPaint_undo" title="Desfazer"></div>').click(function() { $canvas.undoPrev(); })).append($('<div class="_wPaint_icon _wPaint_redo" title="Refazer"></div>').click(function() { $canvas.undoNext(); })); break;
	                //case 'clear': menuContent.append($('<div class="_wPaint_icon _wPaint_clear" title="clear"></div>').click(function(){ $canvas.clearAll(); })); break; 
	                //case 'open': menuContent.append($('<div id="div_open" class="_wPaint_icon _wPaint_open" title="Abrir nova imagem"></div>')); break; 
	                case 'save': menuContent.append($('<div id="div_save" class="_wPaint_icon _wPaint_save" title="Salvar"></div>')); break;
	                //case 'resize': menuContent.append($('<div id="div_resize" class="_wPaint_icon _wPaint_resize" title="Redimensionar área de desenho"></div>')); break; 
	                case 'rectangle': menuContent.append($('<div class="_wPaint_icon _wPaint_rectangle" title="Retângulo"></div>').click(function() { _self.set_mode(_self, $canvas, 'Rectangle'); })); break;
	                case 'ellipse': menuContent.append($('<div class="_wPaint_icon _wPaint_ellipse" title="Círculo"></div>').click(function() { _self.set_mode(_self, $canvas, 'Ellipse'); })); break;
	                case 'line': menuContent.append($('<div class="_wPaint_icon _wPaint_line" title="Linha"></div>').click(function() { _self.set_mode(_self, $canvas, 'Line'); })); break;
	                case 'pencil': menuContent.append($('<div class="_wPaint_icon _wPaint_pencil" title="Lápis"></div>').click(function() { _self.set_mode(_self, $canvas, 'Pencil'); })); break;
	                case 'text': menuContent.append($('<div class="_wPaint_icon _wPaint_text" title="Texto horizontal"></div>').click(function() { $('#hidden_tipo_texto_selecionado').val('H'); _self.set_mode(_self, $canvas, 'Text'); })); break;
	                case 'text_v': menuContent.append($('<div class="_wPaint_icon _wPaint_text_V" title="Texto vertical"></div>').click(function() { $('#hidden_tipo_texto_selecionado').val('V'); _self.set_mode(_self, $canvas, 'Text_V'); })); break;
	                case 'eraser': menuContent.append($('<div class="_wPaint_icon _wPaint_eraser" title="Borracha"></div>').click(function(e) { _self.set_mode(_self, $canvas, 'Eraser'); })); break;
	                case 'fillColor': menuContent.append($('<div class="_wPaint_strokeColorPicker _wPaint_colorPicker" title="Cor borda/texto"></div>')); break;
	                case 'lineWidth': menuContent.append(lineWidth); break;
	                case 'strokeColor': menuContent.append($('<div class="_wPaint_fillColorPicker _wPaint_colorPicker" title="Cor preenchimento"></div>')); break;
	                //case 'biblioteca_imagens': menuContent.append($('<div id="div_biblioteca_imagens" class="_wPaint_icon _wPaint_biblioteca_imagens" title="Biblioteca de imagens">Biblioteca imagens</div>')); break; 
	                case 'imagem_padrao':
	                    if ($("#hidden_imagem_alterada").val() == true) {
	                        menuContent.append($('<div class="_wPaint_icon _wPaint_imagem_padrao" title="Voltar imagem para o padrão do projeto">Voltar padrão</div>').click(function(e) { if (confirm('Deseja realmente voltar esta imagem para o padrão do projeto?')) { fnc_operacao_imagem('voltar_padrao', $("#hidden_serial").val()); } })); break;
	                    }
	            }
	        });
	    ...
	    set_mode: function(_self, $canvas, mode) {
	        // Esconde as caixas de upload e redimensionamento
	        $('.box_upload_imagem').hide();
	        $('.box_redimensionar_area_desenho').hide();

	        if (mode == 'Text_V') {
	            $canvas.settings.mode = 'Text';
	        }
	        else {
	            $canvas.settings.mode = mode;
	        }

	        if (mode == 'Text' || mode == 'Text_V') {
	            $("._wPaint_textInput").val('');

	            switch (mode) {
	                case 'Text': $("._wPaint_textInput").css('transform', 'rotate(0deg)'); break;
	                case 'Text_V': $("._wPaint_textInput").css('transform', 'rotate(-90deg)'); break;
	            }

	            _self.textMenu.menu.show();
	            _self.setWidth(_self.textMenu.menu);

	            $("._wPaint_textInput").moveable({ img: $('#hidden_contexto').val() + "/geral/biblioteca/editor_imagem_js/arrow.png" });

	            $("#img_change_position_text").hide();

	            if ($("._wPaint_textInput").is(":visible")) {
	                var left = $("._wPaint_textInput").position().left;
	                var top = $("._wPaint_textInput").position().top;
	        ...
	        if (mode == 'Text_V') {
	            _self.menu.find("._wPaint_text_V").addClass('active');
	        }
	        else {
	            ...
	            switch (mode) {
	                case 'Ellipse':
	                case 'Line':
	                case 'Rectangle': $('body').css('cursor', 'crosshair'); break;

	                case 'Eraser': $('body').css('cursor', 'url(../../Images/EdicaoImagemProjeto/cursor_borracha.png),url(../../Images/EdicaoImagemProjeto/cursor_borracha.png),auto'); break;

	                case 'Pencil': $('body').css('cursor', 'url(../../Images/EdicaoImagemProjeto/pencil.png),url(../../Images/EdicaoImagemProjeto/pencil.png),auto'); break;

	                default: $('body').css('cursor', 'default'); break;
	            }
	        }
	    },
    ...
    */
    ...
    TextMenu.prototype =
	{
	    generate: function(canvas, mainMenu) {
	        ...
	        var fontSize = $('<div class="_wPaint_fontSize _wPaint_dropDown" title="Tamanho da fonte"></div>').append(
				$('<select>' + options + '</select>')
				.change(function(e) {
	        ...
	        /*var fontFamily = $('<div class="_wPaint_fontFamily _wPaint_dropDown" title="Família de fontes"></div>').append(
	        $('<select>' + options + '</select>')
	        .change(function(e){ 
			...
			.append($('<div class="_wPaint_icon _wPaint_bold ' + ($canvas.settings.fontTypeBold ? 'active' : '') + '" title="Negrito"></div>').click(function() { _self.setType(_self, $canvas, 'Bold'); }))
	        //.append($('<div class="_wPaint_icon _wPaint_italic ' + ($canvas.settings.fontTypeItalic ? 'active' : '') + '" title="Itálico"></div>').click(function(){ _self.setType(_self, $canvas, 'Italic'); }))
			.append($('<div class="_wPaint_icon _wPaint_underline ' + ($canvas.settings.fontTypeUnderline ? 'active' : '') + '" title="Sublinhado"></div>').click(function() { _self.setType(_self, $canvas, 'Underline'); }))
			.append(fontSize)
	        ...
	        /*.append(menuHandle)*///Movimenta barra
			.append(menuContent);
	    },
