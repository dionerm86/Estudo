...
namespace Glass.API.Backend.Controllers.Log.V1
{
    ...
    public partial class LogController : BaseController
    {
        ...
        [SwaggerResponse(200, "Logs sem paginação (apenas uma página de retorno) ou última página retornada.", Type = typeof(IEnumerable<LogDto>))]
        [SwaggerResponse(204, "Logs não encontrados para o filtro informado.")]
        [SwaggerResponse(206, "Logs paginados (qualquer página, exceto a última).", Type = typeof(IEnumerable<LogDto>))]
        [SwaggerResponse(400, "Filtro inválido informado (campo com valor ou formato inválido).", Type = typeof(MensagemDto))]
        [ResponseType(typeof(IEnumerable<LogDto>))]
        public IHttpActionResult ObterLogsAlteracao([FromUri] FiltroLogAlteracaoDto filtro)
        {
            using (var sessao = new GDATransaction())
            {
                ...
                return this.ListaPaginada(
                    logs.Select(log => new LogDto(log)),
                    filtro,
