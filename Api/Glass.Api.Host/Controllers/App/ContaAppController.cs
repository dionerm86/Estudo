        ...
        public HttpResponseMessage ObterImagensModelos()
        {
            ...
            var streamContent = new PushStreamContent((stream, content, context) =>
            {
                using (var zipFile = new Ionic.Zip.ZipFile())
                {
                    if (System.IO.Directory.Exists(diretorioModelosProjeto))
                    {
                        foreach (var i in System.IO.Directory.GetFiles(diretorioModelosProjeto))
                        {
                            if (!i.Contains("§") && !i.Contains("%"))
                            {
                                var entry = zipFile.AddFile(i, "/");
                                entry.FileName = Colosoft.Text.StringExtensions.RemoveDiacritics(entry.FileName);
                            }
                        }
                    }
